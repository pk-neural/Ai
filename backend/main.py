...
fruit_session = ort.InferenceSession(
    "models/fruit_model-2.onnx",
    providers=["CPUExecutionProvider"]
)

document_session = ort.InferenceSession(
    "models/resnet18_aadhaar.onnx",
    providers=["CPUExecutionProvider"]
)

fastfood_session = ort.InferenceSession(
    "models/food_detection_model.onnx",
    providers=["CPUExecutionProvider"]
)

# ✅ DEEPFAKE MODEL (NEW – SAME STYLE)
deepfake_session = ort.InferenceSession(
    "models/rvf10k_resnet18.onnx",
    providers=["CPUExecutionProvider"]
)
...
async def scan_fruit(file: UploadFile = File(...)):
    ...
    return {
        "prediction": FRUIT_CLASSES[pred_class],
...
async def scan_document(file: UploadFile = File(...)):
    ...
    return {
        "prediction": DOCUMENT_CLASSES[pred_class],
        "confidence": round(confidence, 2)
    }
...
async def scan_fastfood(file: UploadFile = File(...)):
    ...
    if len(inp_shape) == 4 and inp_shape[-1] == 3:
        img_tensor = np.transpose(img_tensor, (0, 2, 3, 1))  # NHWC
    ...
    return {
        "prediction": FASTFOOD_CLASSES[pred_class],
        "confidence": round(confidence, 2)
    }
...
async def scan_deepfake(file: UploadFile = File(...)):
    ...
    # Handle sigmoid or softmax models
    if out.ndim == 2 and out.shape[1] == 1:
    ...
    return {
        "prediction": DEEPFAKE_CLASSES[pred_class],
